...
#epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+)\sdot\s([A-Za-z]+)\sdot\sedu')
...
#ouster - teresa.lynn (followed by "@stanford.edu")
epatterns.append('([A-Za-z.]+)\s\(followed by "@([A-Za-z.]+)\.edu')
epatterns.append('([A-z]+)\s+\(followed by\s?(?:&ldquo;)?@([A-z.-]+)+\.edu')
...
#for hager
lpatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+)\sdot\s([A-Za-z]+)\sdot\sedu')
...
#This part is added to match the email addresses that end with .com
...
#This part of the function is added to match the 3 parts of the email addresses
